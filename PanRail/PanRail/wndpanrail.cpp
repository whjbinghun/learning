...
{
    QRect rct_rail, rct_ptz, rct_set;//用rect确定某种空间的区域位置，从而省去了窗口控件的使用
    ...
    if ( mb_rail_hidden ) {
        ...
        if ( mb_rail_ver_hor ) {//垂直
            if ( mb_rail_lefttop_rightbottom ) {//左
                rct_rail = QRect( SPACE/2, 0, SPACE, height() - HEIGHT_SET_AREA );
                rct_ptz = QRect( SPACE*3/2, 0, width()-SPACE*3/2, height() - HEIGHT_SET_AREA );
            } else { //右
                rct_ptz = QRect( 0, 0, width()-SPACE*3/2, height() - HEIGHT_SET_AREA );
                rct_rail = QRect( width()-SPACE*3/2, 0, SPACE, height() - HEIGHT_SET_AREA );
            }
        } else { //水平
            if ( mb_rail_lefttop_rightbottom ) {//上
                rct_rail = QRect( 0, SPACE/2, width(), SPACE);
                rct_ptz = QRect( 0, SPACE*3/2, width(), height() - HEIGHT_SET_AREA - SPACE*3/2 );
                qDebug()<<"WndPanRail::update_ctrl_pos"<<width()<<height();
            } else {//下
                rct_ptz = QRect( 0, 0, width(), height() - HEIGHT_SET_AREA - SPACE*3/2 );
                rct_rail = QRect( 0, height() - HEIGHT_SET_AREA- SPACE*3/2, width(), SPACE );
            }
        }
        rct_set = QRect( 0, height() - HEIGHT_SET_AREA, width(), HEIGHT_SET_AREA );
    }
...
void WndPanRail::move_ptz_ctrl( QRect rct )
{
    ...
    int n_width_space = (rct.width()-BTN_WIDTH*3)/4;
    int n_height_space = (rct.height()-BTN_HEIGHT*3)/4;

    mp_btn_left->move( n_x+n_width_space, n_y+n_height_space*2+BTN_HEIGHT );
    mp_btn_left->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_lefttop->move( n_x+n_width_space, n_y+n_height_space );
    mp_btn_lefttop->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_top->move( n_x+n_width_space*2+BTN_WIDTH, n_y+n_height_space );
    mp_btn_top->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_righttop->move( n_x+n_width_space*3+BTN_WIDTH*2, n_y+n_height_space );
    mp_btn_righttop->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_right->move( n_x+n_width_space*3+BTN_WIDTH*2, n_y+n_height_space*2+BTN_HEIGHT);
    mp_btn_right->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_rightbottom->move( n_x+n_width_space*3+BTN_WIDTH*2, n_y+n_height_space*3+BTN_HEIGHT*2 );
    mp_btn_rightbottom->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_bottom->move( n_x+n_width_space*2+BTN_WIDTH, n_y+n_height_space*3+BTN_HEIGHT*2 );
    mp_btn_bottom->resize( BTN_WIDTH, BTN_HEIGHT );
    mp_btn_leftbottom->move( n_x+n_width_space, n_y+n_height_space*3+BTN_HEIGHT*2 );
    mp_btn_leftbottom->resize( BTN_WIDTH, BTN_HEIGHT );
}
...
void WndPanRail::move_rail_ctrl( QRect rct )
{
    if ( mb_rail_ver_hor ) {//垂直
        ...
        int n_slider_hieght = rct.height() - ( n_btn_height + SPACE ) * 2;

        mp_btn_ver_top->move( rct.left(), rct.top() + SPACE );
        mp_btn_ver_top->resize( n_btn_width, n_btn_height );

        mp_slider_ver->move( rct.left(), rct.top() + SPACE + n_btn_height );
        mp_slider_ver->resize( n_slider_width, n_slider_hieght );

        mp_btn_ver_bottom->move( rct.left(), rct.top() + SPACE + n_btn_height + n_slider_hieght );
        mp_btn_ver_bottom->resize( n_btn_width, n_btn_height );
    } else {
        int n_btn_width = SPACE;
        int n_btn_height = rct.height();

        int n_slider_width = rct.width()-( n_btn_width + SPACE )*2;
        int n_slider_hieght = n_btn_height;

        mp_btn_hor_left->move( rct.left() + SPACE, rct.top() );
        mp_btn_hor_left->resize( n_btn_width, n_btn_height );

        mp_slider_hor->move( rct.left() + SPACE + n_btn_width, rct.top());
        mp_slider_hor->resize( n_slider_width, n_slider_hieght );

        mp_btn_hor_right->move( rct.left() + SPACE + n_btn_width + n_slider_width, rct.top() );
