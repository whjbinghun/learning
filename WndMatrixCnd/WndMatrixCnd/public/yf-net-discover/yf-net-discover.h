...
#pragma pack(push)
#pragma pack(1)
typedef struct tagSrvDiscDevInfo_S{
    int  i_dev_type;
    char   ac_sn[32];
    unsigned int ui_ipv4_addr;
    unsigned char auc_ipv6[16];
    unsigned short us_port;
    unsigned char  auc_mac[6];
}SrvDiscDevInfo_S;
#pragma pack(pop)
...
//p_user_data:用户自定义数据
typedef bool ( *NET_DISCOVER_CB_FUNC )( SrvDiscDevInfo_S *pst_dev_info, void *p_user_data );
//网络设备发现库服务端初始化
//h:网络设备发现库句柄
