...
void QtTreePropertyBrowserPrivate::init(QWidget *parent)
{
    ...
    QStringList labels;
    labels.append(QApplication::translate("QtTreePropertyBrowser", "属性", 0));
    labels.append(QApplication::translate("QtTreePropertyBrowser", "值", 0));
    m_treeWidget->setHeaderLabels(labels);

    //m_treeWidget->setHeaderHidden( true );//隐藏头
...
void QtTreePropertyBrowser::setResizeMode(QtTreePropertyBrowser::ResizeMode mode)
{
    ...
    d_ptr->m_treeWidget->header()->setResizeContentsPrecision( m );// ->setResizeMode(m);
}
