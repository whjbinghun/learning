...
void WndPalette::get_pelete_info( bool &b_auto, float &f_min_range, float &f_max_range, float &f_level, float &f_levelspan, ColorIndex &clr )
{
    QMutexLocker lc( &m_mutex );
    b_auto = mb_auto_palatte;
    f_min_range = mf_min;
    f_max_range = mf_max;
    f_level = mf_level;
    f_levelspan = mf_level_span;
    clr = m_clr_index;
}
...
void WndPalette::paintEvent( QPaintEvent *event )
{
    ...
    if( !mb_hor_ver ) {
        ...
        rct_adjust_bar = QRect( 0, rct_top.bottom() + 3, rct.right()+1, rct_bottom.top() - rct_top.bottom() - 4 );
    } else {
        ...
        rct_adjust_bar = QRect( n_btn_width+2, 0, rct.right()-n_btn_width*4-7, rct.height() );
    }
    draw_backgroud( draw );
...
//    }
    ...
    draw.drawRect( rct );
    draw.setPen( QColor( 255, 255, 255 ) );
    draw.drawText( rct, Qt::AlignCenter, "自动" );
}
...
{
    if ( mb_mouse_press || !mb_auto_palatte ) return;
...
void WndPalette::set_level_span(float f_level, float f_level_span)
{
    mf_level = f_level;
    mf_level_span = f_level_span;
    mb_recal_levelspan = true;
