...
{
    QString str_color = QString( "[%1, %2, %3]" )
            .arg(QString::number(c.red()))
            .arg(QString::number(c.green()))
            .arg(QString::number(c.blue()));
    return QApplication::translate("QtPropertyBrowserUtils", str_color.toLatin1().data() );
...
{
    QString str_font = QString( "[%1, %2]" ).arg(f.family())
            .arg(f.pointSize());
    return QApplication::translate("QtPropertyBrowserUtils", str_font.toLatin1().data(), 0 );
}
