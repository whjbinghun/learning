...
bool SuSqlite::create_table( QString str_table_name )
{
    ...
    if( m_sql_query.exec( QObject::tr( str_create_table.toStdString().c_str() ) ) ) {
        qDebug()<<"创建数据库表成功";
...
bool SuSqlite::insert_data( int n_id, QString str_name )
{
    ...
    if( m_sql_query.exec( QObject::tr( str_insert.toStdString().c_str() ) ) ) {
        qDebug()<<"插入数据成功";
        return true;
    }
...
bool SuSqlite::update_data( int n_id, QString str_name )
{
    ...
    if( m_sql_query.exec( QObject::tr( str_update.toStdString().c_str() ) ) ) {
        qDebug()<<"更新数据成功";
        return true;
    }
...
{
    QString str_delete = QString( "delete into %1 where id = %2 " ).arg( ms_sql_table_name ).arg( n_id );
    if( m_sql_query.exec( QObject::tr( str_delete.toStdString().c_str() ) ) ) {
        qDebug()<<"删除数据成功";
        return true;
    }
...
bool SuSqlite::query_all_data()
{
    ...
    if( m_sql_query.exec( QObject::tr( str_query_all_data.toStdString().c_str() ) ) )   //尝试列出  表的所有记录
    {
        while( m_sql_query.next() ) {//开始就先执行一次next()函数，那么query指向结果集的第一条记录
            PersonInfo person_info;
            person_info.n_id = m_sql_query.value( 0 ).toInt();
            person_info.str_name = m_sql_query.value( 1 ).toString();
            person_info.str_month = m_sql_query.value( 2 ).toString();
...
bool SuSqlite::query_data( int n_id )
{
    ...
    if( m_sql_query.exec( QObject::tr( str_query_data.toStdString().c_str() ) ) )   //尝试列出  表的所有记录
    {
        qDebug()<<"查询数据成功";
        return true;
    }
